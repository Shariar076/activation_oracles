...
def run_oracle(
    ...
    # Generate the target model's own response (target adapter is still active here)
    target_output_ids = model.generate(**inputs_BL, max_new_tokens=100, do_sample=False, temperature=0.0)
    target_response = tokenizer.decode(
        target_output_ids[0][inputs_BL["input_ids"].shape[1]:], skip_special_tokens=True
    )
...
results_path = Path(__file__).resolve().parent / "elk_results.json"


def format_target_prompt(user_msg: str) -> str:
...
def compute_user_msg_segment(formatted: str, user_msg: str) -> tuple[int, int | None]:
    """Segment covers tokens from the end of the user message through the end of the formatted prompt
    (i.e. <|im_end|>, the assistant opener, and the empty think block)."""
...
# Load oracle adapter once
load_lora_adapter(model, oracle_lora_path)

all_results = []
total = len(WORDS) * len(PROMPTS)
run_idx = 0
...
for word in WORDS:
    ...
    for prompt_text in PROMPTS:
        run_idx += 1
        formatted_target_prompt = format_target_prompt(prompt_text)
        ...
        all_results.append({
            "word": word,
            "prompt": prompt_text,
            "formatted_prompt": formatted_target_prompt,
